...
[`report.html`](../../src/cloud_portable_s3tests/report) report.

```
.venv/bin/python examples/htmlreport/main.py \
...
[`s3tests` CLI](../../src/cloud_portable_s3tests/_cli.py) does the same thing
(and streams console progress while running):
...
(`make samples` from `packages/python/` regenerates both):
