...
func TestAssertions(t *testing.T) {
	cases := []struct {
		...
		{"containsAll", `{"$containsAll":[{"Key":"b"},{"Key":"a"}]}`, []any{map[string]any{"Key": "a"}, map[string]any{"Key": "b", "X": 1}}, true, true},
		{"containsAll one missing", `{"$containsAll":["a", "z"]}`, []any{"a", "b"}, true, false},
		{"containsAll empty list", `{"$containsAll":[]}`, []any{"a"}, true, true},
		{"containsAll not an array", `{"$containsAll":["a"]}`, "a", true, false},
		{"containsAll absent", `{"$containsAll":["a"]}`, nil, false, false},
		{"containsAll with length", `{"$length":2,"$containsAll":["a", "b"]}`, []any{"a", "b"}, true, true},
		{"containsAll with length wrong", `{"$length":3,"$containsAll":["a", "b"]}`, []any{"a", "b"}, true, false},
