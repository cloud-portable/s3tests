...
test('assertion operators', () => {
  for (const [name, expected, actual, present, ok] of [
    ...
    ['containsAll', { $containsAll: [{ Key: 'b' }, { Key: 'a' }] }, [{ Key: 'a' }, { Key: 'b', X: 1 }], true, true],
    ['containsAll one missing', { $containsAll: ['a', 'z'] }, ['a', 'b'], true, false],
    ['containsAll empty list', { $containsAll: [] }, ['a'], true, true],
    ['containsAll not an array', { $containsAll: ['a'] }, 'a', true, false],
    ['containsAll absent', { $containsAll: ['a'] }, undefined, false, false],
    ['containsAll with length', { $length: 2, $containsAll: ['a', 'b'] }, ['a', 'b'], true, true],
    ['containsAll with length wrong', { $length: 3, $containsAll: ['a', 'b'] }, ['a', 'b'], true, false],
